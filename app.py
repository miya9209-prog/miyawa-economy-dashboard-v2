...
def safe_series(x) -> pd.Series:
    if x is None: return pd.Series(dtype="float64")
...
if st.session_state.get("show_guide", False) and hasattr(st, "dialog"):
    @st.dialog("활용법 가이드")
    def guide():
        st.markdown("""
- **보기 단위(좌측)**: 일간(변동) / 주간(추세) / 월간(사이클)  
- **정규화 100**: 시작점을 100으로 맞춰 “상대 강도” 비교  
- **부동산(KOSIS)**: KOSIS 서버가 느릴 수 있어 재시도/축소 호출로 안정화  
""")
        if st.button("닫기"):
...
# Tabs
# =========================
tabs = st.tabs(["시장 한눈에", "내 관심종목", "부동산(KOSIS)", "경제뉴스"])
...
with tabs[2]:
    ...
    st.caption("KOSIS 서버가 느릴 수 있어 재시도/자동 축소 호출로 안정화했습니다.")
    ...
    if not api_key or not tbl_id or not itm_id:
        ...
        # 자동 축소 후보 (40,000셀/대용량 회피)
        candidates = [12, 6, 3]  # 월간이면 12->6->3 으로 줄이며 시도
        desired = st.select_slider("가져올 기간(최근 개수)", options=[3,6,12], value=6)
        # 사용자가 선택한 값을 우선 시도하게 앞에 배치
        candidates = [desired] + [c for c in candidates if c != desired]
        ...
        for cnt in candidates:
            ...
            if err is not None:
                ok_note = f"네트워크/타임아웃: {err.get('errMsg','')}"
                continue
            if isinstance(data, dict) and "err" in data:
                # 40,000셀 에러면 cnt를 더 줄여서 재시도
                ok_note = f"KOSIS 에러: {data.get('err')} / {data.get('errMsg','')}"
                continue
            if isinstance(data, list) and len(data) > 0:
                ok_df = pd.DataFrame(data)
                ok_note = f"호출 성공 (newEstPrdCnt={cnt})"
                break
            ok_note = "데이터가 비어있습니다."
        ...
        if ok_df is None or ok_df.empty:
            ...
            st.info(ok_note)
            st.info("이 tblId는 지역값이 너무 많아 ALL 조회가 제한될 수 있습니다. (40,000셀/응답지연)")
        else:
            st.success(ok_note)
            date_col, val_col, nm_cols = pick_cols(ok_df)

            if date_col is None or val_col is None:
                st.warning("시계열 변환에 필요한 컬럼을 찾지 못했습니다. raw를 확인해주세요.")
                with st.expander("raw 상위 50행"):
                    st.dataframe(ok_df.head(50), use_container_width=True)
            else:
                # 지역 컬럼이 있으면 pivot, 없으면 단일
                if nm_cols:
                    region_col = None
